...
class SettingController extends Controller
{
    ...
    public function updateSiteInfo(Request $request) {
        ...
        if ($request->has('loginBg')) {
            $image = $request->file('loginBg');
            $name = 'loginBg.png';
            $path = public_path('/images/site');
            $image->move($path, $name);
        }
        ...
        foreach ($request->except(['_token','site_logo','favicon','default']) as $key => $value) {
        ...
        addReport(auth()->user()->id, 'بتحديث بيانات التطبيق', $request->ip());
        return back()->with('success', 'تم تعديل بيانات التطبيق');
    }
