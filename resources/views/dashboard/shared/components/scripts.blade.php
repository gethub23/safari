    ...
    {{-- edit form  --}}
        <script>
            $(document).on('click','.edit',function(e){
                ...
                var data = $(this).data();
                @if(!isset($withRoutes))
                    $('#edit_form').attr('action', '{{$routeName}}/'+data.id);
                @else
                    var url = '{{ route("$updateRoute", ":id") }}';
            ...
            $.ajax({
                ...
                success: function( msg ) {
                    ...
                    if(rowCount == 0){
                        ...
                        $('.Block_Up').append('<img  src="{{url('public/design/admin/images/no_found.png')}}">')
                        $('.Block_Up').append('</div>')
